...
var randomNumber = Math.floor(Math.random() * 6) + 1; //genere un chiffre aleatoire entre 1 et 8 pour les images
...
document.querySelector(".btn-roll").addEventListener("click", function() {
    if (boolPlay) {
        ...
        if (randomNumber == 1 && numberTmp == 6) {
            ...
            document.querySelector(".current-score-" + indexPlayer).textContent = scoreTmp;
        } else { //GHOST on est tombé sur 3 ,on passe a lautre joueur,on met tous les points à 0 
...
document.querySelector(".btn-hold").addEventListener("click", function() {
    ...
    if (boolPlay) { //boolPlay = true
        ...
        //sil est gagne = 200
        if (scoreActuel[indexPlayer - 1] >= 100) {
